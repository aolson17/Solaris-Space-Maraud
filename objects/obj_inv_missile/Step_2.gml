if object_exists(par_rack) && distance_to_object(par_rack) = 0 {
x = link.x
y = link.y
} else {
    instance_destroy()
...
if room != ShipBuilder {
if keyboard_check(vk_tab) && distance_to_object(obj_player) <= 40{
    visible = true
} else {
    visible = false
}
...
if mouse_check_button_pressed(mb_left) && distance_to_point(mouse_x,mouse_y) = 0 && distance_to_object(obj_player) <= 40 && keyboard_check(vk_tab){
